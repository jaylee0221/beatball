...
const PACK_TEAMS=B.teams.filter(t=>t.players.filter(id=>E.P[id]&&E.P[id].capPct!=null).length>=8);
...
const zW=(p,k,v)=>{const m=B.weights.SM[p.season+':'+k]; return m?(v-m[0])/m[1]:0;};
...
function cardHTML(p,opts={}){ const [tw,tc]=tagOf(p); const tier=tierOf(p), L=TIER_LOOK[tier]; const first=p.name.split(' ').slice(0,-1).join(' '), ln=last(p);
  ...
  return `<div class="kc narrow ${opts.cls||''}" style="--bgc:${bg}" ${opts.attr||''}>
    <div class="khead"><span class="kprice ${opts.over?'over':''}">${p.capPct!=null?`$${price(p)}<small>M</small>`:'<small>NO SALARY</small>'}</span><button class="kdots" data-kinfo="${p.id}" aria-label="profile">···</button></div>
...
/* ---------- render + events ---------- */
function render(){ const app=$('app'); let body='';
...
load(); S.team=null; S.season=null; S.view='home'; S.tab='build';   // roguelike: every launch starts from the wheel. Only the record persists.
render();
