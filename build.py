...
<style>{css}</style></head><body><div id="app"><div style="padding:40px 20px;font:500 14px/1.5 -apple-system,Archivo,sans-serif;color:#8A9099">Loading Fantasyball…</div></div><pre id="bootlog" hidden></pre><script>window.__t0=Date.now();window.__log=function(m){{var b=document.getElementById("bootlog");if(b)b.textContent+="\\n"+m+" · "+(Date.now()-window.__t0)+"ms";}};window.onerror=function(m,s,l,c){{window.__log("ERROR "+m+" @ "+l+":"+c);}};window.__log("boot 1 · scripts start");</script>
<script>{bundle}</script><script>window.__log("boot 2 · data parsed, "+BEATBALL_V2.players.length+" men");</script><script>window.__log('boot 2 · data parsed, '+BEATBALL_V2.players.length+' men');</script>
<script>{eng}</script><script>window.__log("boot 3 · engine loaded");</script><script>window.__log('boot 3 · engine loaded');</script>
{TRAP}
<script>try{{{app}
